...
CMD ["mkdir -p /root/.openclaw /data/workspace && echo \"$OPENCLAW_CONFIG_B64\" | base64 -d > /root/.openclaw/openclaw.json && cat /root/.openclaw/openclaw.json && OPENCLAW_STATE_DIR=/root/.openclaw exec node dist/index.js"]
